...
flowchart TD
    ...
    B -->|Consulta| C[Respuesta<br/>informativa]
    B -->|Positivo| D[Agradecer +<br/>invitar a volver]
    B -->|Negativo| E[Disculparse +<br/>mover a privado]
    B -->|Spam| F[Ignorar/Eliminar]

    C --> G[¿Requiere<br/>seguimiento?]
    G -->|Sí| H[Pedir datos<br/>por DM]
    G -->|No| I[Cerrar con<br/>CTA amigable]

    D --> J[Invitar a<br/>compartir review]

    E --> K[Resolver<br/>en privado]
...
graph TD
    ...
    B --> B1["✓ Confirmación reserva<br/>✓ Pre-llegada<br/>✓ Post-checkout<br/>✓ Factura"]

    C --> C1["✓ Ofertas temporada<br/>✓ Paquetes especiales<br/>✓ Flash sales<br/>✓ Early bird"]

    D --> D1["✓ Bienvenida base datos<br/>✓ Cumpleaños<br/>✓ Aniversario estadía<br/>✓ Re-engagement inactivos"]
...
sequenceDiagram
    ...
    Note over H,P: RESERVA REALIZADA

    P->>E: Trigger: Nueva reserva
    E->>H: Email 1: Confirmación (inmediato)

    Note over H,P: 7 DÍAS ANTES

    E->>H: Email 2: Pre-llegada + Upselling

    Note over H,P: 1 DÍA ANTES

    E->>H: Email 3: Recordatorio + Check-in online

    Note over H,P: DURANTE ESTADÍA

    E->>H: Email 4: Bienvenida + Tips (día 1)

    Note over H,P: 1 DÍA DESPUÉS CHECK-OUT

    E->>H: Email 5: Agradecimiento + Review request

    Note over H,P: 30 DÍAS DESPUÉS

    E->>H: Email 6: "Te extrañamos" + Oferta retorno

    Note over H,P: 1 AÑO DESPUÉS

    E->>H: Email 7: Aniversario + Descuento especial
```
...
graph TD
    ...
    B --> B1["✓ Awareness/Inspiración<br/>✓ Remarketing<br/>✓ Lookalike audiences<br/>Budget: $10-50/día"]

    C --> C1["✓ Captar búsquedas activas<br/>✓ Google Hotels<br/>✓ Display remarketing<br/>Budget: $20-100/día"]

    D --> D1["✓ Audiencia joven<br/>✓ Contenido viral<br/>✓ Brand awareness<br/>Budget: $20-50/día"]
