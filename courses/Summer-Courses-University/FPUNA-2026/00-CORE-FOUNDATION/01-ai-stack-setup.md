...
- `@anthropic-ai/claude-code` = El nombre del programa
...
```bash
claude "Hola, ¿puedes confirmar que estás funcionando? Responde en español."
```
...
```bash
claude "Crea una calculadora simple en JavaScript que:
...
```bash
claude "Crea un archivo prueba.js que use la calculadora con ejemplos"
```
...
- [ ] ✅ `claude "hola"` responde correctamente
...
#### 1. "comando no encontrado: claude"
...
```bash
claude "Tu pregunta aquí. RESPONDE EN ESPAÑOL."
```
...
- `claude "pregunta"` = Preguntarle a tu IA
