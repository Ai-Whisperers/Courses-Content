# 🤖 Prompts - Investigación y Academia

## FPUNA Verano 2026

Colección de prompts para investigación académica con Claude/ChatGPT.

---

## Revisión de Literatura

### Búsqueda Sistemática
```
Ayúdame a diseñar una estrategia de búsqueda sistemática para:

Pregunta de investigación: [PICO O SIMILAR]
Campo: [DISCIPLINA]
Bases de datos disponibles: [LISTA]
Período de tiempo: [AÑOS]

Genera:
1. Términos de búsqueda (keywords, sinónimos)
2. Operadores booleanos optimizados
3. Query específico por base de datos
4. Criterios de inclusión/exclusión sugeridos
5. Número estimado de resultados a esperar
```

### Síntesis de Paper
```
Resume el siguiente paper académico:

[PEGAR ABSTRACT O SECCIONES CLAVE]

Proporciona:
1. Objetivo principal del estudio
2. Metodología utilizada (diseño, muestra, instrumentos)
3. Hallazgos principales (con números si los hay)
4. Limitaciones mencionadas
5. Contribución al campo
6. ¿Cómo se relaciona con [MI TEMA DE INVESTIGACIÓN]?
```

### Identificar Gaps
```
Basándote en estos resúmenes de literatura sobre [TEMA]:

[PEGAR RESÚMENES O LISTA DE HALLAZGOS]

Identifica:
1. Temas bien cubiertos
2. Inconsistencias o contradicciones entre estudios
3. Gaps o áreas no exploradas
4. Metodologías dominantes y alternativas no usadas
5. Poblaciones/contextos no estudiados
6. Preguntas de investigación potenciales
```

---

## Metodología

### Diseño de Estudio
```
Necesito diseñar un estudio para investigar:

Pregunta: [PREGUNTA DE INVESTIGACIÓN]
Hipótesis (si aplica): [HIPÓTESIS]
Campo: [DISCIPLINA]
Recursos disponibles: [TIEMPO, PRESUPUESTO, ACCESO]
Población objetivo: [DESCRIBIR]

Sugiere:
1. Tipo de estudio más apropiado (y justificación)
2. Variables (dependiente, independiente, control)
3. Tamaño de muestra recomendado (con cálculo)
4. Método de muestreo
5. Instrumentos de recolección
6. Consideraciones éticas
7. Posibles limitaciones y cómo mitigarlas
```

### Validación de Instrumento
```
Revisa este instrumento de recolección de datos:

[PEGAR PREGUNTAS/ÍTEMS]

Objetivo del instrumento: [DESCRIBIR]
Población objetivo: [DESCRIBIR]
Constructo a medir: [DESCRIBIR]

Evalúa:
1. Claridad de cada ítem
2. Posibles sesgos de redacción
3. Cobertura del constructo
4. Orden lógico de preguntas
5. Formato de respuesta apropiado
6. Sugerencias de mejora específicas
```

### Guía de Entrevista
```
Crea una guía de entrevista semi-estructurada para:

Tema: [TEMA DE INVESTIGACIÓN]
Participantes: [DESCRIBIR]
Duración objetivo: [MINUTOS]
Enfoque: [exploratorio/confirmatorio]

Incluye:
1. Introducción y consentimiento
2. Preguntas de rapport inicial
3. Preguntas principales por tema (5-7)
4. Sondeos (probes) para cada pregunta
5. Preguntas de cierre
6. Notas para el entrevistador
```

---

## Análisis de Datos

### Plan de Análisis
```
Tengo estos datos de mi estudio:

Variables:
- [VAR1]: tipo, escala
- [VAR2]: tipo, escala
- [etc.]

Pregunta/hipótesis: [DESCRIBIR]
Tamaño de muestra: [N]
Software disponible: [SPSS/R/Python/otro]

Genera un plan de análisis que incluya:
1. Análisis descriptivo por variable
2. Verificación de supuestos
3. Pruebas estadísticas apropiadas
4. Tamaño del efecto a reportar
5. Código/comandos específicos
6. Cómo interpretar y reportar resultados
```

### Interpretación de Resultados
```
Ayúdame a interpretar estos resultados estadísticos:

[PEGAR OUTPUT ESTADÍSTICO]

Contexto del estudio: [BREVE DESCRIPCIÓN]
Hipótesis: [HIPÓTESIS]

Explica:
1. ¿Qué significa cada estadístico?
2. ¿La hipótesis se apoya o rechaza?
3. ¿El efecto es estadísticamente significativo?
4. ¿El efecto es prácticamente significativo (tamaño)?
5. ¿Cómo reportar esto en formato APA?
6. ¿Hay limitaciones en la interpretación?
```

### Visualización de Datos
```
Necesito crear visualizaciones para estos datos:

Datos:
[DESCRIBIR VARIABLES Y ESTRUCTURA]

Mensaje a comunicar: [QUÉ QUIERO MOSTRAR]
Audiencia: [académica/general/presentación]
Herramienta: [Python/R/otro]

Sugiere:
1. Tipo(s) de gráfico más apropiado(s)
2. Variables en cada eje
3. Código para crear el gráfico
4. Elementos a incluir (título, leyenda, etc.)
5. Buenas prácticas de visualización científica
```

---

## Escritura Académica

### Estructura de Sección
```
Ayúdame a estructurar la sección de [INTRODUCCIÓN/MÉTODOS/RESULTADOS/DISCUSIÓN] para mi paper sobre:

Tema: [DESCRIBIR]
Revista objetivo: [NOMBRE/TIPO]
Extensión aproximada: [PALABRAS]

Genera:
1. Outline detallado (párrafos y puntos clave)
2. Transiciones entre subsecciones
3. Información que debe incluirse
4. Información que debe evitarse
5. Primer párrafo de ejemplo
```

### Mejorar Párrafo
```
Mejora este párrafo académico:

[PEGAR PÁRRAFO]

Objetivos de mejora:
- Claridad
- Concisión
- Flujo lógico
- Voz académica apropiada
- Eliminación de redundancias

Proporciona:
1. Versión mejorada
2. Explicación de cada cambio
3. Sugerencias adicionales
```

### Abstract
```
Escribe un abstract para este paper:

Título: [TÍTULO]
Objetivo: [OBJETIVO]
Método: [RESUMEN DE METODOLOGÍA]
Resultados principales: [HALLAZGOS]
Conclusión: [IMPLICACIONES]

Requisitos:
- Límite de palabras: [150-300]
- Estilo: [estructurado/narrativo]
- Keywords sugeridas: [5-7]

Genera 2 versiones para elegir.
```

### Parafrasear
```
Parafrasea el siguiente texto para evitar plagio mientras mantienes el significado:

Original:
"[TEXTO]"

Fuente: [AUTOR, AÑO]

Proporciona:
1. Paráfrasis apropiada
2. Cómo citarlo correctamente
3. Alternativas de redacción
4. ¿Es mejor citar directamente?
```

---

## Presentaciones

### Estructura de Presentación
```
Ayúdame a estructurar una presentación de [MINUTOS] sobre:

Tema: [TÍTULO DEL PAPER/TESIS]
Audiencia: [conferencia/defensa/clase]
Hallazgos principales: [LISTA]

Genera:
1. Estructura de slides (título + contenido clave)
2. Puntos de habla por slide
3. Visualizaciones sugeridas
4. Posibles preguntas de la audiencia
5. Tips para esa audiencia específica
```

### Responder Preguntas
```
Prepárame para responder preguntas sobre mi investigación:

Resumen del estudio: [BREVE DESCRIPCIÓN]
Metodología: [MÉTODO]
Limitaciones conocidas: [LISTA]
Resultados principales: [LISTA]

Genera:
1. 10 preguntas probables y respuestas sugeridas
2. Preguntas difíciles sobre metodología
3. Cómo responder "no sé" profesionalmente
4. Cómo deflectar a trabajo futuro
```

---

## Publicación

### Selección de Revista
```
Ayúdame a seleccionar revistas para enviar mi paper:

Tema: [DESCRIBIR]
Campo: [DISCIPLINA]
Tipo de estudio: [cuantitativo/cualitativo/mixto]
Novedad: [incremental/significativo/breakthrough]
Prisa: [alta/media/baja]
Open access: [requerido/preferido/no importa]

Considerando:
1. Impact factor deseado
2. Tiempo de revisión típico
3. Tasa de aceptación
4. Costo de publicación

Sugiere 5 revistas rankeadas con justificación.
```

### Cover Letter
```
Escribe una cover letter para enviar mi paper a [REVISTA]:

Título del paper: [TÍTULO]
Resumen del estudio: [2-3 oraciones]
Contribución principal: [DESCRIBIR]
Por qué esta revista: [RAZÓN]

Incluye:
1. Saludo apropiado
2. Descripción concisa del paper
3. Relevancia para la revista
4. Declaración de originalidad
5. Información de contacto
6. Cierre profesional
```

### Respuesta a Revisores
```
Ayúdame a responder estos comentarios de revisores:

Comentario del Revisor [X]:
"[COMENTARIO]"

Mi situación:
- [Estoy de acuerdo / En desacuerdo / Parcialmente]
- [Lo que puedo/no puedo hacer]

Genera:
1. Respuesta profesional y constructiva
2. Cambios específicos a hacer en el manuscrito
3. Cómo manejar desacuerdos respetuosamente
```

---

## Ética y Buenas Prácticas

### Verificar Ética
```
Revisa mi estudio desde la perspectiva ética:

Diseño: [DESCRIBIR ESTUDIO]
Participantes: [POBLACIÓN]
Datos a recolectar: [TIPOS]
Riesgos potenciales: [LISTA]

Evalúa:
1. ¿Se necesita aprobación ética?
2. Elementos del consentimiento informado
3. Protección de datos personales
4. Posibles daños y beneficios
5. Consideraciones especiales (poblaciones vulnerables)
6. Checklist de ética para mi campo
```

---

*Prompts - Track 05 - FPUNA 2026*
